...
contract DollarsTest is Test {
    ...
    function testPrizePoolDualAccounting() public {
        ...
        assertEq(
            _dollars.getActualAvailable(),
            _paymentToken.balanceOf(address(_dollars)),
            "Actual available should match contract balance"
        );
    }
    ...
    function testClaimPrizeAccountingUpdates() public {
        ...
        vm.mockCall(
            address(_dollars), abi.encodeWithSelector(_dollars.isWinningToken.selector, tokenId), abi.encode(true)
        );
        vm.stopPrank();
